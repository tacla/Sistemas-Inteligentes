...
def gerar_vizinhos2(sudoku, modifiable_positions):
    """
    Estratégia 2: troca diretamente dois valores modificáveis.
    Retorna as posições trocadas.
    """
    estrategia = random.choice(['linha', 'coluna', 'bloco'])

    grupos = {}
    for i, j, _ in modifiable_positions:
        if estrategia == 'linha':
            grupos.setdefault(i, []).append((i, j))
        elif estrategia == 'coluna':
            grupos.setdefault(j, []).append((i, j))
        else:
            bloco_id = (i // 3, j // 3)
            grupos.setdefault(bloco_id, []).append((i, j))

    grupos_validos = [g for g in grupos.values() if len(g) >= 2]
    if not grupos_validos:
        return None

    grupo = random.choice(grupos_validos)
    (i1, j1), (i2, j2) = random.sample(grupo, 2)

    # troca direta
    v1 = sudoku.board[i1][j1]
    v2 = sudoku.board[i2][j2]
    sudoku.play(i1, j1, v2)
    sudoku.play(i2, j2, v1)

    return (i1, j1, v1), (i2, j2, v2)
...
# === Random restart ====
calc_vizinhanca = 2
for rs in range(1, 10000):
    # === Preenchimento inicial aleatório ===
    for i, j, _ in modifiable_positions:
        s.play(i, j, random.randint(1, 9))
    print("\n-----------------------------------------------------")
    print(f"Preenchimento aleatório inicial (tentativa {rs})")
    prev_violations = s.violations()
